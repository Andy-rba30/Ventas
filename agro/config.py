"""Constantes de configuración de la aplicación."""

NOMBRE_APP = "Sistema Agro-Negocio Familiar"
# Nombre del archivo de la BD; la carpeta la decide agro.rutas (nunca junto al ejecutable).
NOMBRE_BD = "negocio_final_stock.db"

CLIENTE_GENERAL = "PÚBLICO GENERAL"
ENCARGADA_DEFAULT = "Administradora"

# Un producto con stock disponible igual o menor se resalta en rojo (mínimo por defecto).
UMBRAL_BAJO_STOCK = 5

# Unidades de medida disponibles al crear o editar un producto.
UNIDADES = ["unid", "kg", "L", "saco", "bolsa"]

MODO_APARIENCIA = "Light"
TEMA_COLOR = "blue"
GEOMETRIA_INICIAL = "1200x800"
